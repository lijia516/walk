...
inline void ModelerUI::cb_addNewPt_i(Fl_Light_Button*, void*)
{
    if (m_pbtAddNewPt->value() == 1) {
      ...
      //  myik->mBodys.push_back(rshin);
        ...
        myik->mFootPosition(0) = ParticleSystem::footPosition[0];
        myik->mFootPosition(1) = myik->mPelPostion(1) - 2.3 * cos(0.314);
        myik->mFootPosition(2) = myik->mPelPostion(2) - 2.3 * sin(0.314);
    ...
    std::cout << "AddNewPt " << m_pbtAddNewPt->value() <<std::endl;
}
